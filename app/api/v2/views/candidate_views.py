...
candidatee = Blueprint('candidate', __name__)


class CandidateEndPoint:
    """Candidate API Endpoints"""

    @candidatee.route('/office/<int:office>/register', methods=["POST"])
    #@admin_required
    def register(office):
    ...
    @candidatee.route('/office/<int:office>/deregister', methods=["POST"])
    #@admin_required
    ...
    @candidatee.route('/office/<int:office>/politicians', methods=["GET"])
    #@admin_required
    ...
    @candidatee.route('/candidates', methods=["GET"])
    #@jwt_required
    def get_candidates():
        """ Get all approved candidates endpoint """
        return success(200, "Success", Candidate().get_all_candidates())

    @candidatee.route('office/<int:id>/candidates', methods=["GET"])
    #@jwt_required
